...
class Model_app extends CI_Model
{
	...
	public function subdepartemen()
	{
		...
		$query = $this->db->query("SELECT * FROM bagian_departemen A LEFT JOIN departemen B ON B.id_dept = A.id_dept ORDER BY nama_bagian_dept desc");
		return $query;
	}
	...
	public function daftar_tugas($id)
	{
		...
		$query = $this->db->query("SELECT A.progress, A.status, A.id_ticket, A.reported, A.tanggal, A.tanggal_solved, A.id_kondisi, A.deadline,
                                   ...
                                   WHERE F.nik = '$id' AND A.status IN (4,6,7) ORDER BY A.deadline ASC");
		return $query;
	}
	...
	public function update($id)
	{
		...
		$config['allowed_types']  = 'gif|jpg|png|pdf';  //Tipe gambar yang diizinkan
		$config['max_size']       = '2000';     //Ukuran maksimum file gambar yang diizinkan
	...
	public function getkodeticket()
	{
		...
		$max_id = $row['max_code'];
		//Membuat format kode ticket dengan dengan memulai kode dari posisi 9 dan panjang kode 4
		...
		//Membuat id_ticket dengan format T + kode user terbaru
