...
class Dashboard extends CI_Controller
{
	...
	public function index()
	{
		...
		$tek_assign = $this->db->query("SELECT COUNT(A.id_ticket) AS jlm_tekassign FROM ticket A 
                                         LEFT JOIN user B ON B.username = A.teknisi
                                         LEFT JOIN karyawan C ON B.username = C.nik 
                                         WHERE C.nik = '$id_user'")->row();
		$data['tekassign'] = $tek_assign->jlm_tekassign;
		//Jumlah tiket yang perlu di approve tiap teknisi
		$tek_approve = $this->db->query("SELECT COUNT(A.id_ticket) AS jlm_tekapprove FROM ticket A 
                                         LEFT JOIN user B ON B.username = A.teknisi
                                         LEFT JOIN karyawan C ON B.username = C.nik 
                                         WHERE A.status = 3 AND C.nik = '$id_user'")->row();
		$data['tekapprove'] = $tek_approve->jlm_tekapprove;
		//Jumlah tiket yang dikerjakan tiap teknisi
		$tek_kerja   = $this->db->query("SELECT COUNT(A.id_ticket) AS jlm_tekkerja FROM ticket A 
                                         LEFT JOIN user B ON B.username = A.teknisi
                                         LEFT JOIN karyawan C ON B.username = C.nik 
                                         WHERE A.status = 4 AND C.nik = '$id_user'")->row();
		$data['tekkerja'] = $tek_kerja->jlm_tekkerja;
		//Jumlah tiket yang dipending tiap teknisi
		$tek_pending = $this->db->query("SELECT COUNT(A.id_ticket) AS jlm_tekpending FROM ticket A 
                                         LEFT JOIN user B ON B.username = A.teknisi
                                         LEFT JOIN karyawan C ON B.username = C.nik 
                                         WHERE A.status = 5 AND C.nik = '$id_user'")->row();
		$data['tekpending'] = $tek_pending->jlm_tekpending;
		//Jumlah tiket yang selesai dikerjakan tiap teknisi
		$tek_selesai = $this->db->query("SELECT COUNT(A.id_ticket) AS jlm_tekselesai FROM ticket A 
                                         LEFT JOIN user B ON B.username = A.teknisi
                                         LEFT JOIN karyawan C ON B.username = C.nik 
                                         WHERE A.status = 6 AND C.nik = '$id_user'")->row();
